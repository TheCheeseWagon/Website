...
// === Instagram Grid Loader (resilient mount + proxy-first) ===
(function(){
  function ready(fn){
  ...
  function pickMount(){
    var m = document.getElementById('ig-feed') ||
            document.querySelector('.instagram-embed') ||
            document.querySelector('#social .instagram-embed') ||
            document.querySelector('.social-grid');
    if (m) return m;
    // Fallback: create a mount so preview-only pages can render too
    var parent = document.getElementById('social') || document.body;
    var div = document.createElement('div');
    div.id = 'ig-feed';
    // minimal styles so it doesn't look broken in CSS-only preview
  ...
  function init(){
    ...
    detectPreview().then(function(isPreview){
      var imgBases = isPreview
        ? ['/proxy/images/', abs(liveBase, 'images/'), 'https://thecheesewagon.github.io/Website/images/']
        : [abs(liveBase, 'images/'), 'https://thecheesewagon.github.io/Website/images/'];

      var jsonList = isPreview
        ? [jsonViaProxy, jsonPrimary, jsonFallback]
        : [jsonPrimary, jsonFallback];

      function firstJSON(list, i){
        i = i || 0;
        if (i >= list.length) return Promise.reject(new Error('no-json'));
        return tryFetch(list[i]).catch(function(){ return firstJSON(list, i+1); });
      }
      firstJSON(jsonList).then(function(data){
        var items = (data && (data.items || data.slots)) || [];
        if (!items.length){
          mount.innerHTML = '<p style=\"text-align:center;opacity:.8;\">No featured photos yet.</p>';
          return;
        }
        function pickImgBase(bases, testFile, i){
          i = i || 0;
          if (i >= bases.length) return Promise.resolve(bases[0]);
          var url = (/^(https?:|data:)/i.test(testFile)) ? testFile : bases[i] + testFile;
          return new Promise(function(res){
            var img = new Image();
            img.onload = function(){ res(bases[i]); };
            img.onerror = function(){ res(null); };
            img.src = url;
          }).then(function(ok){ return ok ? ok : pickImgBase(bases, testFile, i+1); });
        }
        var firstFile = toFilename(items[0] && (items[0].src || items[0].path || items[0]));
        pickImgBase(imgBases, firstFile).then(function(imgBase){
