...
async function bootstrapSessionOnce(username, password) {
  // One-time bootstrap: send header tokens on a GET (no CSRF required), receive cookies + X-CSRF header
  const resp = await fetch(APIBASE + "/menu", {
    method: "GET",
    credentials: "include",
    headers: { "x-admin-token": password, "x-username-token": username }
  });
  // Read CSRF from response header (Worker exposes X-CSRF on bootstrap)
  const csrf = resp.headers.get("x-csrf");
  if (csrf) { window.__CSRF__ = csrf; }
